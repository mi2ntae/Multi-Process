...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "4":
					...
					break;
				case "5":
					eventBus.sendEvent(new Event(EventId.DeleteStudent, deleteById("Student")));
					printLogSend(EventId.DeleteStudent);
					break;
				case "6":
					eventBus.sendEvent(new Event(EventId.DeleteCourse, deleteById("Course")));
				...
				case "8":
					...
					printLogSend(EventId.ListEnrollments);
					break;
				case "0":
	...
	private static String deleteById(String object) throws IOException {
		System.out.println("\nEnter "+object+" ID and press return (Ex. 20131234)>> ");
		return new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
	}
	private static String enroll() throws IOException {
		String userInput = "";
		System.out.println("\nEnter student ID and press return (Ex. 20131234)>> ");
		userInput = new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
		System.out.println("\nEnter course ID and press return (Ex. 12345)>> ");
	...
	private static void writeMenu() {
		...
		System.out.println("8. List Enrollment");
